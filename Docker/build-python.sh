...
buildContainers() {
  echo; echo "Building container: pythondemo-app"
  (cd app; docker build -t appdynamics/pythondemo-app:${VERSION} .) || exit $?
}

copyAgents() {
  echo "Copying agent installers"
  cp .appdynamics/MachineAgent.zip app/
}

# Usage information
if [[ $1 == *--help* ]]
then
  echo "Specify agent locations: build-python.sh
          -m <Path to Machine Agent>
          [-b Build pythondemo-app image]"
...
then   
  ...
  # Download latest Python Agent, MA, and JA Installers from download.appdynamics.com
  ...
  then
    ...
    # Allow user to specify locations of Python, Machine and Java Agent Installers
